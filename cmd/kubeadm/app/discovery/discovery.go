...
func For(cfg *kubeadmapi.NodeConfiguration) (*clientcmdapi.Config, string, error) {
...
func GetValidatedClusterInfoObject(cfg *kubeadmapi.NodeConfiguration) (*clientcmdapi.Cluster, error) {
	...
	case len(cfg.DiscoveryToken) != 0:
		clusterInfo, _, _, _, err := token.RetrieveValidatedClusterInfo(cfg.DiscoveryToken, cfg.DiscoveryTokenAPIServers, cfg.DiscoveryTokenCACertHashes)
