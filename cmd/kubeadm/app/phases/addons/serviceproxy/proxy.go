...
func CreateTenxProxyAddon(cfg *kubeadmapi.MasterConfiguration, client clientset.Interface) error {
...
func createTenxProxy(certsConfigMapBytes, configMapBytes, daemonSetBytes []byte, client clientset.Interface) error {
	...
	if err := kuberuntime.DecodeInto(legacyscheme.Codecs.UniversalDecoder(), certsConfigMapBytes, certsConfigMap); err != nil {
	...
	if err := kuberuntime.DecodeInto(legacyscheme.Codecs.UniversalDecoder(), configMapBytes, tenxProxyConfigMap); err != nil {
